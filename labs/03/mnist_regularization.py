...
# with zero l2.
def get_regularizer():
    if args.l2 is None or args.l2 == 0: regularizer = None
    else: regularizer = tf.keras.regularizers.L1L2(l2=args.l2)
    return regularizer
...
if args.dropout: model.add(tf.keras.layers.Dropout(args.dropout))
for hidden_layer in args.hidden_layers:
    model.add(tf.keras.layers.Dense(hidden_layer, activation=tf.nn.relu, bias_regularizer=get_regularizer(), kernel_regularizer=get_regularizer()))
    if args.dropout: model.add(tf.keras.layers.Dropout(args.dropout))
model.add(tf.keras.layers.Dense(MNIST.LABELS, bias_regularizer=get_regularizer(), kernel_regularizer=get_regularizer()))
...
model.fit(
    mnist.train.data["images"][:5000], tf.keras.utils.to_categorical(mnist.train.data["labels"][:5000]),
    batch_size=args.batch_size, epochs=args.epochs,
    validation_data=(mnist.dev.data["images"], tf.keras.utils.to_categorical(mnist.dev.data["labels"])),
    callbacks=[tb_callback],
)

test_logs = model.evaluate(mnist.test.data["images"], tf.keras.utils.to_categorical(mnist.test.data["labels"]), batch_size=args.batch_size)
