...
#                                     (Caltech42.MIN_SIZE, Caltech42.MIN_SIZE, Caltech42.C), np.float32))
    ...
    def __init__(self, positive_class, negative_class, augmentation, preprocessing, folds):
        """
        Parameters
        ----------
        ...
        positive_idx = self.labels.index(positive_class)
        negative_idx = self.labels.index(negative_class)
        ...
        self.test = self.Dataset(positive_idx, self.test, preprocessing, shuffle_batches=False)
        self.train["images"] += self.dev["images"]
        self.train["labels"] = np.concatenate((self.train["labels"], self.dev["labels"]))
        del self.dev

        positive_mask = self.train["labels"] == positive_idx
        negative_mask = self.train["labels"] == negative_idx
        mask = np.logical_or(positive_mask, negative_mask)
        self.train["images"] = np.array(self.train["images"])[mask]
        self.train["labels"] = np.array(self.train["labels"])[mask]
        ...
        for subset in subsets:
            self.folds.append(self.CrossvalidationFold(positive_idx, subset['train'], subset['dev'], preprocessing, augmentation))
