...
class Network:
    def __init__(self, args):
        ...
        for i in range(len(args.generator_layers)):
            z = tf.keras.layers.Dense(args.generator_layers[i], activation='relu')(z)
        z = tf.keras.layers.Dense(MNIST.H * MNIST.W * MNIST.C, activation='sigmoid')(z)
        generator_output = tf.keras.layers.Reshape([MNIST.H, MNIST.W, MNIST.C])(z)
        ...
        for i in range(len(args.discriminator_layers)):
            x = tf.keras.layers.Dense(args.discriminator_layers[i], activation='relu')(x)
