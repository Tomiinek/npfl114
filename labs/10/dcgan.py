...
class Network:
    def __init__(self, args):
        ...
        z = tf.keras.layers.Dense(1024, activation=None, use_bias=False)(generator_input)
        z = tf.keras.layers.BatchNormalization()(z)
        z = tf.keras.layers.ReLU()(z)

        z = tf.keras.layers.Dense(MNIST.H // 4 * MNIST.W // 4 * 64, activation=None, use_bias=False)(z)
        z = tf.keras.layers.BatchNormalization()(z)
        z = tf.keras.layers.ReLU()(z)

        z = tf.keras.layers.Reshape([MNIST.H // 4, MNIST.W // 4, 64])(z)

        z = tf.keras.layers.Conv2DTranspose(filters=32, kernel_size=5, strides=2, padding='same', use_bias=False, activation=None)(z)
        z = tf.keras.layers.BatchNormalization()(z)
        z = tf.keras.layers.ReLU()(z)

        generator_output = tf.keras.layers.Conv2DTranspose(filters=1, kernel_size=5, strides=2, padding='same', activation='sigmoid')(z)
        self.generator = tf.keras.Model(inputs=generator_input, outputs=generator_output)
        ...
        discriminator_input = tf.keras.layers.Input(shape=[MNIST.H, MNIST.W, MNIST.C])

        x = tf.keras.layers.Conv2D(filters=32, kernel_size=5, padding='same', use_bias=False, activation=None)(discriminator_input)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.ReLU()(x)

        x = tf.keras.layers.MaxPool2D(pool_size=2, strides=2)(x)

        x = tf.keras.layers.Conv2D(filters=64, kernel_size=5, padding='same', use_bias=False, activation=None)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.ReLU()(x)
        ...
        x = tf.keras.layers.Dense(1024, activation=None, use_bias=False)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.ReLU()(x)

        discriminator_output = tf.keras.layers.Dense(1, activation='sigmoid')(x)
        self.discriminator = tf.keras.Model(inputs=discriminator_input, outputs=discriminator_output)
    ...
    def train_batch(self, images):
        ...
        with tf.GradientTape() as tape:
            ...
            discriminator_loss = self._loss_fn(tf.ones_like(discriminated_real), discriminated_real) + \
                                 self._loss_fn(tf.zeros_like(discriminated_fake), discriminated_fake)
