...
class WideResNet(tf.keras.Model):
    ...
    def train(self, checkpoint_path, args, cifar):
        self.fit_generator(
            generator=cifar.train.mixed_batches(args.directions, args.batch_size, repeat=True),
            steps_per_epoch=(len(args.directions) * cifar.train.size) // args.batch_size,
            epochs=args.epochs,
            validation_data=cifar.dev.mixed_batches(args.directions, args.batch_size, repeat=True),
            validation_steps=(len(args.directions) * cifar.dev.size + args.batch_size - 1) // args.batch_size,
    ...
    def evaluate(self, ds, args):
        ...
        for epoch_i in range(args.eval_epochs):
            for batch_i, (batch, _) in tqdm(
                enumerate(ds.mixed_batches(args.directions, args.batch_size, do_augment=(args.eval_epochs != 1 and epoch_i != 0))),
                leave=False, desc=f"{epoch_i + 1}/{args.eval_epochs}"
            ):
                begin = (batch_i * args.batch_size) % ds.size
                if (begin + len(batch)) > ds.size:
                    batch_probas = self.predict_on_batch(batch)
                    probas[begin:] += batch_probas[:(ds.size - begin)]
                    probas[:(len(batch) - (ds.size - begin))] = batch_probas[(ds.size - begin):]
                else:
                    end = begin + len(batch)
    ...
    def _build(self):
        ...
        x = inputs = Input(shape=(32, 32, 3), dtype=tf.float32)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_epochs", default=8, type=int, help="Number of augmented amples to average over during evaluation")
    ...
    parser.add_argument("--directions", default=[0, 1, 2], type=json.loads, help="List of projection directions to sample from input.")
    ...
    if args.train:
        checkpoint_path = os.path.join("wideresnet_models_mono", "tlustoprd_28-10_{}_{}_{}_{}".format("directions=" + ''.join(map(str, args.directions)), "acc={val_accuracy:.4f}", args.depth, args.spek_factor))
