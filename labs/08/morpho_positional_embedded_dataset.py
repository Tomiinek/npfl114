...
class MorphoDataset:
    ...
    class Dataset:
        ...
        def __init__(self, data_file, embedding_file, train=None, shuffle_batches=True, add_bow_eow=False,
        ...
        def batches(self, size=None):
            ...
            while len(permutation):
                ...
                batch.append(MorphoDataset.FactorBatch(np.zeros([batch_size, max_sentence_len], np.int32),
                                                       np.zeros([batch_size, max_sentence_len, 256], np.float32)))
                for i in range(batch_size):
                    batch[0].word_embeddings[i, :len(self._data[self.FORMS].word_embeddings[batch_perm[i]]), :] = \
                    self._data[self.FORMS].word_embeddings[batch_perm[i]]
                    batch[0].word_ids[i, :len(self._data[self.FORMS].word_ids[batch_perm[i]])] = \
                    self._data[self.FORMS].word_ids[batch_perm[i]]
