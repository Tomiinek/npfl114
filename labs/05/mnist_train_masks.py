...
if __name__ == "__main__":
    ...
    parser.add_argument("--depth", default=98, type=int, help="Depth of the network.")
    parser.add_argument("--alpha", default=73, type=int, help="Widening factor over classical resnet.")
    ...
    checkpoint_path = os.path.join("models-666", "wideresnet_{}-{}_{}".format(args.depth, args.width_factor, "acc={val_class_output_accuracy:.4f}", "iou={val_mask_output_io_u_metric:.4f}"))

    # Load data
    mnist = MNIST(sparse_labels=args.label_smoothing == 0)

    # Create the network and train
    network = PyramidNet(args.depth, args.alpha, args.weight_decay)
    ...
    if args.lr == 'cyclic':
        ...
        learning_rate = SGDRLearningRate(learning_rate=0.125, t_0=60.0 * train_step, m_mul=0.8)
    else:
