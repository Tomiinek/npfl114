...
class Network:
    def __init__(self, args, num_words, num_tags, num_chars):
        ...
        charseq_ids = tf.keras.layers.Input(shape=[None], dtype=tf.dtypes.int32)
        ...
        embedded_charseqs = tf.keras.layers.Embedding(num_chars, args.cle_dim, mask_zero=True)(charseqs)
        embedded_charseqs = tf.keras.layers.Bidirectional(tf.keras.layers.GRU(args.cle_dim, return_sequences=False))(embedded_charseqs)
        ...
        embedded_cle = tf.keras.layers.Lambda(lambda args: tf.gather(*args))([embedded_charseqs, charseq_ids])

        # TODO(we): Embed input words with dimensionality `args.we_dim`, using
        # `mask_zero=True`.
        embedded_we = tf.keras.layers.Embedding(num_words, args.we_dim, mask_zero=True)(word_ids)

        # TODO: Concatenate the WE and CLE embeddings (in this order).
        embeddings = tf.keras.layers.Concatenate()([embedded_we, embedded_cle])
        ...
        # opposite directions.
        x = tf.keras.layers.Bidirectional(self._get_cell(args.rnn_cell, args.rnn_cell_dim))(embeddings)

        # TODO(we): Add a softmax classification layer into `num_tags` classes, storing
        # the outputs in `predictions`.
        predictions = tf.keras.layers.Dense(num_tags, activation='softmax')(x)
    ...
    def train_batch(self, inputs, tags):
        ...
        mask = tf.dtypes.cast(tf.math.not_equal(tags, tf.zeros_like(tags)), dtype=tf.dtypes.float32)
        with tf.GradientTape() as tape:
            probabilities = self.model(inputs, training=True)
            # TODO: Compute `loss` using `self._loss`, passing the generated
            # tag mask as third parameter.
            loss = self._loss(tags, probabilities, sample_weight=mask)
        ...
        with self._writer.as_default():
            for name, metric in self._metrics.items():
                ...
                if name == "loss": metric(loss)
                      ...
                      metric.update_state(tags, probabilities, sample_weight=mask)
                tf.summary.scalar("train/{}".format(name), metric.result())

    def train_epoch(self, dataset, args):
        for batch in dataset.batches(args.batch_size):
            self.train_batch([batch[dataset.FORMS].word_ids, batch[dataset.FORMS].charseq_ids, batch[dataset.FORMS].charseqs], batch[dataset.TAGS].word_ids)
    ...
    def evaluate(self, dataset, dataset_name, args):
         for metric in self._metrics.values():
             metric.reset_states()
         for batch in dataset.batches(args.batch_size):
            self.evaluate_batch([batch[dataset.FORMS].word_ids, batch[dataset.FORMS].charseq_ids, batch[dataset.FORMS].charseqs], batch[dataset.TAGS].word_ids)
