...
class Network:
    def __init__(self, args, num_words, num_tags, num_chars):
        ...
        embedded_charseqs = tf.keras.layers.Embedding(num_chars, args.cle_dim, mask_zero=False)(charseqs)
        convs = []
        for width in range(2, args.cnn_max_width + 1):
            conv = tf.keras.layers.Conv1D(filters=args.cnn_filters, kernel_size=width, activation='relu')(embedded_charseqs)
            convs.append(tf.keras.layers.GlobalMaxPool1D()(conv))
        x = tf.keras.layers.Concatenate()(convs)
        embedded_charseqs = tf.keras.layers.Dense(args.we_dim, activation='relu')(x)
