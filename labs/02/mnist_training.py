...
def get_optimizer(optimizer, decay, training_examples):
    decay_steps = args.epochs * training_examples // args.batch_size
    learning_rate = args.learning_rate
    if decay == 'polynomial':
        learning_rate = tf.keras.optimizers.schedules.PolynomialDecay(args.learning_rate, decay_steps, args.learning_rate_final)
    if decay == 'exponential':
        learning_rate = tf.keras.optimizers.schedules.ExponentialDecay(args.learning_rate, decay_steps, args.learning_rate_final / args.learning_rate)
    m = args.momentum if args.momentum else 0.0
    return {'SGD':  tf.keras.optimizers.SGD(learning_rate=learning_rate, momentum=m),
            'Adam': tf.keras.optimizers.Adam(learning_rate=learning_rate)}.get(optimizer)
...
model.compile(
    optimizer=get_optimizer(args.optimizer, args.decay, mnist.train.size),
